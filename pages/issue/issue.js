	...
	onLoad: function () {
		...
		*/
		...
		db.find().then(res => {
			...
			if (res.mobilePhoneNumber === undefined && res.wechatId === undefined && res().QQ === undefined) {
				that.setData({
					ableToClick: false
				});
			}
	...
	*/
	submit: function () {
		...
		if (that.data.ableToClick) {
			...
			else if (that.data.product_category == "") {
				wx.showModal({
					title: '提示',
					content: '请选择商品类别',
				});
			...
			if (!that.data.user.emailVerified) {
				/*
				如果ableToClick为false
				且用户未验证则提醒用户去验证邮箱
				*/
			...
			else if (that.data.user.mobilePhoneNumber === undefined && that.data.user.wechatId === undefined && that.data.user.QQ === undefined) {
				wx.showModal({
					title: '提示',
